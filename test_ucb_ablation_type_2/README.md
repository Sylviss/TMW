...
- `BASE`: default ablation point (`lambda_ts=0.5`, `mask_type=2`, `eps_threshold=0.2`).
- `BASE_PRETRAIN_EPOCHS`: warm-start epochs (`50`).
- `BASE_PRETRAIN_CONFIG`: pretrain settings (`lambda_ts=0.0`, `mask_type=2`, `eps_threshold=0.2`).
- `SWEEP`: sweep ranges (`lambda_ts=[0, 0.005, 0.01, 0.05, 0.1, 0.5, 1, 5, 10]`, `mask_type=[2]`, `eps_threshold=[0.15, 0.2, 0.3, 0.4, 0.5, 0.7, 1]`).
